...
Once a user has set up a credential (eg. a Pattern lock) and they have recieved their User SID (Secure User Id), they are now ready to authenticate.
...
.. The `gatekeeperd` daemon will send a hash of the password/ping/pattern it received in step 1, to its counterpart `gatekeeper` in the TEE (TEE OS).
